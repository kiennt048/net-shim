...
$backup_file = '/cf/conf/config.xml.pre-reset.' . date('Y-m-d_H-i-s');

// Get the default config content from environment
$default_config = getenv('NETSHIM_DEFAULT_CONFIG');
if (empty($default_config)) {
    die("ERROR:NO_DEFAULT_CONFIG");
}

// Validate XML
libxml_use_internal_errors(true);
$xml = simplexml_load_string($default_config);
if ($xml === false) {
    $errors = libxml_get_errors();
    libxml_clear_errors();
    die("ERROR:INVALID_DEFAULT_CONFIG - " . (isset($errors[0]) ? $errors[0]->message : "Parse failed"));
}

// Check if it looks like a pfSense config
if (!isset($xml->version) || !isset($xml->system)) {
    die("ERROR:INVALID_DEFAULT_CONFIG - missing version or system section");
}
...
if (file_exists($config_file)) {
    if (!copy($config_file, $backup_file)) {
        syslog(LOG_WARNING, "NetShim: Failed to create backup before reset");
        die("ERROR:BACKUP_FAILED");
    }
    syslog(LOG_INFO, "NetShim: Created pre-reset backup at $backup_file");
}

// Write default config
$bytes_written = file_put_contents($config_file, $default_config);
...
if ($written_content !== $default_config) {
    // Restore backup
    if (file_exists($backup_file)) {
        copy($backup_file, $config_file);
    }
...
write_config("NetShim: Configuration reset to factory defaults");

// Apply system configuration
syslog(LOG_INFO, "NetShim: Applying default configuration...");
...
syslog(LOG_INFO, "NetShim: Factory reset completed successfully");
