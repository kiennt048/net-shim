...
// DNSRedirectStatus represents the DNS redirect rule state
type DNSRedirectStatus struct {
...
// AdGuardConfig represents configuration needed for AdGuard automation
...
// DoHDoTStatus represents DoH/DoT blocking rule status
type DoHDoTStatus struct {
	AliasPresent bool `json:"alias_present"`
	DoHPresent   bool `json:"doh_present"`
	DoTPresent   bool `json:"dot_present"`
...
// GetDNSRedirectStatus returns current DNS redirect rule status
func GetDNSRedirectStatus(iface, target, targetPort string) (*DNSRedirectStatus, error) {
	output, err := runPhpRaw(PhpDNSRedirectScript, map[string]string{
		"ACTION":      "get",
		"INTERFACE":   iface,
		"TARGET":      target,
...
// SetDNSRedirect enables or disables the DNS redirect rule
func SetDNSRedirect(iface, target, targetPort string, enable bool) error {
	enabledStr := "0"
	if enable {
		enabledStr = "1"
	}

	_, err := runPhp(PhpDNSRedirectScript, map[string]string{
		"ACTION":      "set",
		"INTERFACE":   iface,
		"TARGET":      target,
		"TARGET_PORT": targetPort,
...
// GetDoHDoTStatus returns current DoH/DoT blocking rule status
func GetDoHDoTStatus(iface string) (*DoHDoTStatus, error) {
	output, err := runPhpRaw(PhpDoHDotBlockScript, map[string]string{
		"ACTION":    "get",
		"INTERFACE": iface,
	})
	if err != nil {
...
// SetDoHDoT enables or disables DoH/DoT blocking rules
func SetDoHDoT(iface string, enableDoH, enableDoT bool, hosts []string) error {
	enabledDoH := "0"
	enabledDoT := "0"
	if enableDoH {
		enabledDoH = "1"
	...
	_, err := runPhp(PhpDoHDotBlockScript, map[string]string{
		"ACTION":     "set",
		"INTERFACE":  iface,
		"ENABLE_DOH": enabledDoH,
		"ENABLE_DOT": enabledDoT,
		"DOH_HOSTS":  strings.Join(hosts, ","),
...
// AdGuardInstall installs and configures AdGuard Home
func AdGuardInstall(cfg AdGuardConfig) (string, error) {
	...
	return runPhpRaw(PhpAdGuardManageScript, env)
}

// AdGuardConfigure updates AdGuard Home configuration
func AdGuardConfigure(cfg AdGuardConfig) (string, error) {
	env := map[string]string{
		"ACTION":        "configure",
		"ADMIN_USER":    cfg.AdminUser,
		"ADMIN_PASS":    cfg.AdminPass,
...
// AdGuardVerify returns diagnostic info for AdGuard Home
func AdGuardVerify(dnsPort, webPort string) (string, error) {
	return runPhpRaw(PhpAdGuardManageScript, map[string]string{
		"ACTION":   "verify",
		"DNS_PORT": dnsPort,
		"WEB_PORT": webPort,
	})
...
// AdGuardDebug returns recent AdGuard log lines
func AdGuardDebug() (string, error) {
	return runPhpRaw(PhpAdGuardManageScript, map[string]string{
		"ACTION": "debug",
	})
}

// AdGuardRestart restarts AdGuard Home service
func AdGuardRestart() (string, error) {
	return runPhpRaw(PhpAdGuardManageScript, map[string]string{
		"ACTION": "restart",
	})
}
